...
func (w *WeChat) GetAccessToken(userCode string) (*Response, error) {
	apiUrl := fmt.Sprintf("https://%s/v2/openlogin?Action=GetAccessTokenByCode&SecretId=%d&SecretKey=%s&UserCode=%s", w.ApiUrl, w.SecretId, w.SecretKey, userCode)
...
func (w *WeChat) FlashToken(c context.Context, currentToken string, expire time.Duration, result chan string) {
	go func() {
		...
		for {
			select {
			case <-timer.C:
				url := fmt.Sprintf("https://%s/v2/openlogin?RefreshAccessToken&SecretId=%d&SecretKey=%s&CurrentAccessToken=%s", w.ApiUrl, w.SecretId, w.SecretKey, currentToken)
